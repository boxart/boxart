 ...
 */
export default class AnimatedAgentBase {
  constructor() {
  ...
  didMount(domNode) {
    if (this.clientRender) {
      ...
      AnimatedRect.getBoundingClientRect(domNode, this.rect);
    }
  }
  ...
  didUpdate() {
    if (this.clientRender) {
      ...
      AnimatedRect.getBoundingClientRect(this.domNode, this.rect);
...
Object.assign(AnimatedAgentBase, (function() {
  let agent, agentNode;

  return {
    globalAgent() {
      if (!agent) {
        agent = new AnimatedAgentBase();
        agentNode = document.querySelector('body');
        agent.didMount(agentNode);
      }
      return agent;
    },
    ...
    /*
     * resetGlobalAgent
     * Reset the global agent. Only use this in testing.
     */
    resetGlobalAgent() {
      if (agent) {
        agent.willUnmount();
